...
# Extracted from your JS array to map Device Name to Model ID
...
def process_bulk_device_update(records):
    ...
    # 1. Fetch all extensions to map extNumber -> extId (Identical to your old JS logic)
    ...
    for row in records:
        ...
        try:
            # Find the target device on the extension to update
            ext_id = ext_map[ext_num]
            devices_data = client.get(f'/restapi/v1.0/account/~/extension/{ext_id}/device').json()
    ...
    try:
        ...
        bulk_response = client.post('/restapi/v1.0/account/~/device/bulk-update', json=bulk_payload).json()
        
        # 4. Map the bulk results back to our tracking list
        for api_result in bulk_response.get('records', []):
            dev_id = str(api_result.get('id', ''))
            # Find the corresponding entry in our results array
            for r in results:
